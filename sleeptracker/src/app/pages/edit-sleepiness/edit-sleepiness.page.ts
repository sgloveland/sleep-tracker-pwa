...
@Component({
  selector: 'app-edit-sleepiness',
  templateUrl: './edit-sleepiness.page.html',
  styleUrls: ['./edit-sleepiness.page.scss'],
})
export class EditSleepinessPage implements OnInit {
  private ScaleValues = [
  ...
  constructor(private route: ActivatedRoute, private sleepService: SleepService, private alertController: AlertController, private navController: NavController) { }

  ngOnInit() {
    this.route.queryParams.subscribe(params => {
      this.id = params["id"];
      this.selectedValue = +params["loggedValue"];
      this.loggedAt = new Date(params["loggedAt"])
    });
  }
    ...
    const alert = this.selectedValue ? (await this.alertController.create({
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            this.sleepService.updateSleepinessData(this.id, this.selectedValue, this.loggedAt);
